...
//----------------------------------------------------------------------------
// Name:	network_is_reachable 
// Purpose:	Check whether any network connection is up.
// Returns:	True if one or more connections are up.
//----------------------------------------------------------------------------
bool network_is_reachable (void)
{
#define TMPFILE TMPDIR"/.reachablenet"
	system ("ifconfig | grep inet | grep netmask | grep -v 127.0.0.1 | wc -l > "TMPFILE);

	long long nConnections;
	if (!read_longlong_from_file (TMPFILE, &nConnections)) {
		nConnections = 0;
	}
